...
const FormScreen = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(error);
      Alert.alert('Error', 'Failed to generate lesson plan. Please check your API key and try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.header}>OpenAI API Key</Text>
      <TextInput
        style={styles.input}
      ...
      />
      <Text style={styles.disclaimer}>Note: Your API key is used only for this session and is not stored.</Text>
